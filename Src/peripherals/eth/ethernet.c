...
static char *payload = "AAAAAAAAAAAAAAAAAAAAAAAA";
...
int build_packet(uint8_t *buf) {
    ...
    *p++ = 0x00;       // DSCP/ECN
    ...
    uint16_t total_length = 20 + 8 + strlen(payload); // IP + UDP + payload
    *p++ = total_length >> 8;
    *p++ = total_length & 0xff;

    *p++ = 0x12; *p++ = 0x34; // Identification
    *p++ = 0x00; *p++ = 0x00; // Flags + Fragment offset

    *p++ = 64;        // TTL
    *p++ = 17;        // Protocol = UDP

    *p++ = 0x00; *p++ = 0x00; // Header checksum (filled later)

    // Source IP (192.168.1.100)
    *p++ = 192; *p++ = 168; *p++ = 1; *p++ = 100;

    // Destination IP (192.168.1.255 broadcast)
    *p++ = 192; *p++ = 168; *p++ = 1; *p++ = 255;

    // Compute checksum
    uint16_t csum = ip_checksum(ip_start, 20);
    ip_start[11] = csum >> 8;
    ip_start[10] = csum & 0xff;
    ...
    uint16_t udp_length = 8 + strlen(payload);
    ...
    *p++ = 0x00; *p++ = 0x00; // UDP checksum = 0 (valid for IPv4)
    ...
    return (int)(p - buf); // total frame length
}
